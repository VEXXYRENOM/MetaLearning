...
export function LabAnalyticsHUD({ tempHistory, substances }: LabAnalyticsHUDProps) {
  ...
  return (
    <div style={{
      position: "absolute", top: isMobile ? "10px" : "20px", right: isMobile ? "10px" : "20px", width: isMobile ? "160px" : "300px",
      display: "flex", flexDirection: "column", gap: isMobile ? "8px" : "16px", zIndex: 40,
      ...
      <div style={{
        ...
        padding: isMobile ? "10px" : "16px", boxShadow: "0 10px 30px rgba(37,99,235,0.05)"
      }}>
        <h3 style={{ margin: isMobile ? "0" : "0 0 12px", fontSize: isMobile ? "0.75rem" : "0.85rem", color: "#0f1f3d", display: "flex", justifyContent: "space-between" }}>
          <span>♨️ Temp</span>
      ...
      {substances.length > 0 && (
        <div style={{
          ...
          <h3 style={{ margin: isMobile ? "0" : "0 0 12px", fontSize: isMobile ? "0.75rem" : "0.85rem", color: "#0f1f3d" }}>
            📊 Composition
          </h3>
          {!isMobile ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "4px", marginTop: "8px" }}>
              {pieData.map(d => (
                <div key={d.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: "0.65rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                    <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: d.color }}></div>
                    <span style={{ color: "#0f1f3d", fontWeight: 600 }}>{d.name}</span>
                  </div>
                  <span style={{ color: "#6b7280", fontFamily: "monospace" }}>{d.moles.toFixed(2)}m</span>
                </div>
              ))}
            </div>
          )}
