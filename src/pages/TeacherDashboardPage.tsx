  ...
  return (
    <div className="step-container center-content">
      <h3 className="section-title">
        {isArabic ? "اختيار عنوان الدرس" : "Lesson Title"}
      </h3>
      <div className="input-group">
        <label>
          {isArabic ? "أدخل عنوان الدرس الذي ترغب بإنشائه" : "Enter the title for your new lesson"}
        </label>
        <input 
          type="text"
          value={lessonName} 
          onChange={e => onChange(e.target.value)} 
          className="large-input custom-input"
          placeholder={isArabic ? "اكتب اسم الدرس..." : "Enter lesson name..."}
        />
      </div>
      ...
      <button className="btn-primary large mt-6" onClick={onNext} disabled={!lessonName || lessonName.trim() === ""}>
...
function Step6LabPreview({ lessonName, autoExplain, setAutoExplain, onNext }: any) {
  const renderModel = () => {
    // Since we no longer use predefined LESSONS dropdown, we render a generic 3D model
    // or we could check if the user uploaded something. For now, we use a generic placeholder.
    return (
      <group scale={[1.5, 1.5, 1.5]}>
        <GltfScene url="https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/master/2.0/Duck/glTF-Binary/Duck.glb" modelScale={1} selectedName={null} onSelectName={() => {}} xrayMode={false} />
      </group>
    );
  };
    ...
    <div className="step-container lab-step">
      <div className="lab-header">
        <h3 className="section-title text-white">
          {lessonName ? `Preview: ${lessonName}` : "Virtual Lab Preview"}
        </h3>
        <p className="text-gray">
          {lessonName 
            ? `Viewing the 3D assets assigned to "${lessonName}".` 
            : "Prepare your custom lesson content below."}
        </p>
      </div>
