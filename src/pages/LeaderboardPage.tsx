...
export function LeaderboardPage() {
  ...
  return (
    <div style={{
      ...
      color: "#0f1f3d", fontFamily: "'Host Grotesk', system-ui, sans-serif", padding: "2rem"
    }}>
      <div style={{ maxWidth: "800px", margin: "0 auto" }}>

        {/* Header */}
        <div style={{ marginBottom: "2rem" }}>
          <Link to="/student/dashboard" style={{ color: "#2563EB", textDecoration: "none",
            display: "inline-flex", alignItems: "center", gap: "6px", marginBottom: "1.5rem", fontSize: "0.88rem", fontWeight: 600 }}>
            <ArrowLeft size={16} /> {isAr ? "العودة للوحة التحكم" : "Back to Dashboard"}
          </Link>
          <div style={{ textAlign: "center" }}>
            <div style={{ fontSize: "3rem", marginBottom: "0.5rem" }}>🏆</div>
            <h1 style={{ fontSize: "2.2rem", margin: "0 0 0.5rem", fontWeight: 900,
              background: "linear-gradient(135deg,#2563EB,#0EA5E9)", WebkitBackgroundClip: "text",
              WebkitTextFillColor: "transparent", letterSpacing: "-.02em" }}>
              Global Leaderboard
            </h1>
            <p style={{ color: "#6b7280", margin: 0 }}>Top explorers of the MetaLearning universe</p>
          </div>
        </div>
        ...
        {myEntry && myXp && (
          <motion.div
            ...
            style={{
              background: "rgba(37,99,235,0.07)", backdropFilter: "blur(16px)",
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
              <div>
                <p style={{ margin: "0 0 4px", color: "#6b7280", fontSize: "0.8rem" }}>YOUR RANK</p>
                <p style={{ margin: 0, fontSize: "2rem", fontWeight: "bold", color: "#2563EB" }}>#{myRank}</p>
              </div>
              <div>
                <p style={{ margin: "0 0 4px", color: "#6b7280", fontSize: "0.8rem" }}>LEVEL</p>
                <p style={{ margin: 0, fontSize: "2rem", fontWeight: "bold", color: "#F59E0B" }}>{myEntry.level}</p>
              </div>
              <div style={{ flex: 1, minWidth: "200px" }}>
                <p style={{ margin: "0 0 8px", color: "#6b7280", fontSize: "0.8rem" }}>
                  {getLevelTitle(myEntry.level)} — {myEntry.points} XP
                </p>
                <div style={{ background: "rgba(37,99,235,0.1)", borderRadius: "999px", height: "8px" }}>
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${myXp.percent}%` }}
                    transition={{ duration: 1, ease: "easeOut" }}
                    style={{ height: "100%", background: "linear-gradient(90deg, #2563EB, #0EA5E9)",
                      borderRadius: "999px" }}
                  />
                </div>
          ...
          {loading ? (
            Array.from({ length: 5 }).map((_, i) => (
              <div key={i} style={{ height: "80px", borderRadius: "12px",
                background: "rgba(37,99,235,0.04)", animation: "pulse 1.5s ease-in-out infinite" }} />
            ))
          ) : leaders.length === 0 ? (
            ...
            <AnimatePresence>
              {leaders.map((entry, idx) => {
                const rankStyle = RANK_STYLES[entry.rank] ?? {
                  bg: "rgba(255,255,255,0.45)", border: "rgba(255,255,255,0.75)", color: "#6b7280", icon: null
                };
                ...
                return (
                  <motion.div
                    ...
                    style={{
                      background: isMe ? "rgba(37,99,235,0.1)" : rankStyle.bg,
                      backdropFilter: "blur(12px)",
                      WebkitBackdropFilter: "blur(12px)",
                      border: `1px solid ${isMe ? "rgba(37,99,235,0.35)" : rankStyle.border}`,
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                        <span style={{ fontWeight: isMe ? 700 : 500, color: isMe ? "#1d4ed8" : "#1e3a5f",
                    ...
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      ...
                      <div style={{ fontSize: "0.7rem", color: "#9ca3af", display: "flex", alignItems: "center", gap: "3px" }}>
